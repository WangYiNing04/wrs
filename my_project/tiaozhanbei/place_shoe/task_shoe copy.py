...
class MultiCameraShoeTask:
    ...
    def align_shoe_pose(self, base, shoe_model_path, real_pcd, visualize=False):
        """点云配准以修正鞋子姿态，同时可选可视化"""
        ...
        if visualize:
            # 在世界中显示原始模型点云和实际点云
            mgm.gen_pointcloud(shoe_pcd, rgba=np.array([0, 0, 1, 0.5])).attach_to(base)  # 蓝色: 模型
            mgm.gen_pointcloud(real_pcd, rgba=np.array([0, 1, 0, 0.5])).attach_to(base)  # 绿色: 实际点云
        ...
        if visualize:
            mgm.gen_pointcloud(aligned_pcd, rgba=np.array([1, 0, 0, 0.6])).attach_to(base)  # 红色: 配准结果
    ...
    def execute_pick_place(self, pick_pos, place_pos, place_rot, arm, robot, obstacles):
    ...
    def run(self, show_camera=False):
        ...
        # 对鞋子执行ICP配准
        pick_obj_pose = self.align_shoe_pose(base, SHOE_MODEL_PATH, shoe_pcd_real,visualize=True) 
        ...
        print(pick_obj_pose)
        place_obj_rot = rm.rotmat_from_euler(0, 0, np.pi)
        print(place_obj_rot)
