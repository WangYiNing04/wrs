...
class MultiCameraBottleTask:
    ...
    def detect_bottles(self, show=False, eps=0.03, bin_size=0.1):
        """
        多摄像头检测瓶子，并根据空间位置去重
            ...
            try:
                ...
                for (x1, y1, x2, y2), cls_id, conf in zip(
                    ...
                    for point in points_world:
                        x = point[0]
                        # 计算所属的 bin（例如 0.15 → 0.1-0.2 → bin_key="0.1"）
                        bin_key = f"{np.floor(x / bin_size) * bin_size:.1f}"
                        if bin_key not in x_bins:
                            x_bins[bin_key] = 0
                        x_bins[bin_key] += 1

                    # 打印实时统计（调试用）
                    print(f"X轴上点分布（当前帧）: {dict(sorted(x_bins.items()))}")
