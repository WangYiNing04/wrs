...
# ------------------ 3️⃣ 邻域点估计 ------------------
# ------------------ 3️⃣ 邻域点估计 ------------------
def _estimate_point_from_neighborhood(target_pixel, pcd_matrix, neighborhood_size=9, outlier_std_threshold=2.0):
    """
    根据像素点邻域估计更稳定的3D点位置
    - 先在邻域中找有效点
    - 再根据Z深度的中值滤波去掉离群点（1cm容差）
    - 最后返回去噪后的平均3D坐标
    """
    if neighborhood_size % 2 == 0:
        neighborhood_size += 1  # 保证邻域为奇数
    ...
    valid_mask = np.isfinite(neighborhood_points[:, 2]) & (neighborhood_points[:, 2] > 0)
    valid_points = neighborhood_points[valid_mask]
    ...
    inliers = valid_points[np.abs(z_values - depth_median) < 0.01]  # 1cm 容差
...
def detect_keypoints_in_leftarm_frame(yolo_model, color_img, pcd_raw, camera_to_world_mat, neighborhood_size=5):
    ...
    pixels_coord = yolo_detect_injection_pix(yolo_model, color_img, toggle_image=True,confident_threshold=0.1)
    ...
    for p in pixels_coord:
        x,y = p
        if 0 <= y < pcd_matrix.shape[0] and 0 <= x < pcd_matrix.shape[1]:
            pcd_point = pcd_matrix[y, x]  # 注意是 [y, x]，因为图像的行是y，列是x
            print(f"像素 ({x}, {y}) 对应的 3D 点坐标: {pcd_point}")
        else:
            print(f"像素 ({x}, {y}) 超出点云矩阵范围！")
            continue
        est = _estimate_point_from_neighborhood(p, pcd_matrix, neighborhood_size = 9)
        if est is not None:
...
if __name__ == "__main__":
    ...
    # --- 画面过暗修复：手动设置曝光和增益 ---
    try:
        # 关闭自动曝光/增益 (假设 4 是 Auto-Exposure 选项 ID)
        rs_cam.set_option(4, 0)
        # 设置手动曝光 (例如 30000 微秒 = 30 毫秒)
        rs_cam.set_option(5, 30000)  # 假设 5 是 Exposure 选项 ID
        # 设置增益 (例如 100)
        rs_cam.set_option(6, 100)  # 假设 6 是 Gain 选项 ID
        print("RealSense 曝光和增益已手动设置。如果画面仍暗，请尝试调大 30000 和 100。")
    except AttributeError:
        print("警告：RealSenseD400 类可能不支持 set_option 方法，请手动检查相机驱动配置。")

    # Warm-up (保持不变)
    rs_cam.get_pcd_texture_depth()
    rs_cam.get_pcd_texture_depth()

    print("实时检测已启动。按 'q' 键或 'Ctrl+C' 退出。")
