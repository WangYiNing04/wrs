...
class MultiCameraBlockTask:
    ...
    # ------------------------------------------------------------------
    # 新增 align_block_pose
    # (模仿 task_shoes.py 中的 align_shoe_pose)
    # ------------------------------------------------------------------
    def align_block_pose(self, base, block_model_path, real_pcd, visualize=False):
        ...
        model_pcd = model_pcd[model_pcd[:, 2] > 0.004]
 ...
 ``23       """
        ...
        for k in range(len(jv_list) - 1):
            ...
            start_gripper = 0.08 if ev_list[k] >= 0.09 else 0.0
            end_gripper = 0.08 if ev_list[k + 1] >= 0.09 else 0.0
    ...
    def run(self, show_camera=False):
        ...
        for i, color_id in enumerate(color_sequence):
            ...
            filtered_pcd = pcd[(pcd[:, 2] > 0.004) & (pcd[:, 2] < 0.05)]
            if len(filtered_pcd) < 100:
                print(f"⚠️ 方块 {cls_id} 过滤后点云太少, 跳过ICP。")
            else:
                self.align_block_pose(base, BLOCK_MODEL_PATH, filtered_pcd, visualize=True)
            ...
            # (如果需要仿真回放, mot_data也需要附加到base, 但原脚本没有, 故保持不变)
