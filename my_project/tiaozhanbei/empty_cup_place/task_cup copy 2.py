...
class MultiCameraCupTask:
    ...
    # YOLO多摄像头检测 + 点云可视化
    # -------------------------
    def detect_cups(self, show=False, eps=0.03, base=None):
        """
        ...
        for name, cam_info in self.cameras.items():
            ...
            try:
                ...
                for (x1, y1, x2, y2), cls_id, conf in zip(
                    ...
                    # ✅【新增】可视化点云
                    if base is not None:
                        pc_gm = gm.GeometricModel(initor=points_world)
                        # pc_gm.set_rgba([0, 0, 1, 0.4])  # 蓝色半透明点云
                        pc_gm.attach_to(base)
    ...
    def run(self, show_camera=False):
        ...
        # ✅ 传入 base 可视化点云
        objects = self.detect_cups(show=show_camera, base=base if self.visualize else None)
        if not objects:
            print("⚠️ 没有检测到杯子或杯垫！")
            return False
        ...
        end_time = time.time()
        print(f"推理用时 {end_time-time_start:.2f} s")
