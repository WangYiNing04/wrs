...
LastEditTime: 2025-10-23 22:07:39
FilePath: /wrs_tiaozhanbei/my_project/tiaozhanbei/stack_bowls_three/deploy new.py
Description: 三个碗堆叠任务
...
class BowlStackingTask:
    ...
    def __init__(self):
        ...
        self.target_positions = [
            [0.25, -0.30, 0.00],      # 第一个碗放在中间
            [0.25, -0.30, 0.005],      # 第二个碗放在第一个碗上面
            [0.25, -0.30, 0.01]       # 第三个碗放在第二个碗上面
        ]
    ...
    def run_stacking_task(self):
        ...
        if not bowls_positions or len(bowls_positions) < 3:
            print(f"检测到{len(bowls_positions) if bowls_positions else 0}个碗，未检测到全部碗")
