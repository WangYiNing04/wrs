...
class MultiCameraBowlTask:
    ...
    def detect_bowls(self, show=False, eps=0.03):
        ...
        deduped_results.sort(key=lambda x: abs(x[1][1] + 0.3))
    ...
    def _execute_trajectory(self, arm:PiperArmController, mot_data, show_sim=False):
        jv = mot_data.jv_list
        ev = mot_data.ev_list
        ...
        time.sleep(0.1)
        arm.move_jntspace_path(approach_path,speed=15,control_frequency = 0.015)
        time.sleep(0.2)
        arm.close_gripper()
        time.sleep(0.2)
        arm.move_jntspace_path(pick_path,speed=15,control_frequency = 0.015)
        time.sleep(0.2)
        arm.open_gripper(width = 0.03)
        time.sleep(0.2) 
        arm.move_jntspace_path(depart_path,speed=15,control_frequency = 0.015)
