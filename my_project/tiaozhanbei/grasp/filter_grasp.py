...
LastEditTime: 2025-10-30 13:50:15
...
grasp_collection = grasp_collection.load_from_disk(r'/home/wyn/PycharmProjects/wrs_tiaozhanbei/my_project/tiaozhanbei/grasp/grasp_shoe/grasp_shoe.pickle')
...
for grasp in grasp_collection:
    ...
    #     continue
    # if grasp.ac_pos[1] > 0.025 or grasp.ac_pos[1] < -0.025: 
    #     continue
    # if grasp.ac_pos[2] > 0.12 or grasp.ac_pos[2] < 0.02:
    #     continue
    if z_axis_world[2] > -0.01:
        continue
    if grasp.ac_pos[0] > 0.03:
        continue
    if grasp.ac_pos[0] < -0.03:
        continue
    if grasp.ac_pos[1] > 0.03:
        continue
    if grasp.ac_pos[1] < -0.03:
        continue
    if grasp.ac_pos[2] < 0.015:
        continue

    if grasp.ac_pos[2] > 0.07:
        continue

    # # if y_axis_world[1] > 0.0:
    # #     continue
    # if grasp.ee_values < 0.06:
    #     continue
...
# 保存过滤后的抓取集合
filtered_grasp_collection.save_to_disk(r'/home/wyn/PycharmProjects/wrs_tiaozhanbei/my_project/tiaozhanbei/grasp/grasp_shoe/filter_grasp_shoe.pickle')

obj_cmodel = mcm.CollisionModel(r"/home/wyn/PycharmProjects/wrs_tiaozhanbei/0000_examples/objects/shoes.stl")
